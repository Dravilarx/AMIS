...
const InstitutionsModule: React.FC<Props> = ({ isDark, currentUser }) => {
  ...
  const filteredInstitutions = useMemo(() => {
    ...
    if (statusFilter !== 'All') {
      filtered = filtered.filter(i => statusFilter === 'Activo' ? i.active : !i.active);
    }

    return filtered;
  }, [institutions, searchTerm, categoryFilter, statusFilter]);
        ...
        <div className="flex gap-3">
          ...
          <div className="flex border border-slate-300 dark:border-slate-600 rounded-[20px] overflow-hidden">
            <button
              ...
              className={`px-5 py-4 text-[10px] font-black uppercase tracking-widest ${globalViewMode === 'dashboard' ? 'bg-blue-600 text-white' : 'bg-white dark:bg-slate-800 hover:bg-slate-50 dark:hover:bg-slate-700'}`}
            >
              Dashboard
            </button>
            <button
              onClick={() => setGlobalViewMode('table')}
              className={`px-5 py-4 text-[10px] font-black uppercase tracking-widest border-l border-slate-300 dark:border-slate-600 ${globalViewMode === 'table' ? 'bg-blue-600 text-white' : 'bg-white dark:bg-slate-800 hover:bg-slate-50 dark:hover:bg-slate-700'}`}
            >
              Tabla
            </button>
          </div>
        ...
        <div className="space-y-6">
          ...
          <div className={`rounded-[32px] border overflow-hidden ${isDark ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-200 shadow-sm'}`}>
            <div className="overflow-x-auto">
              <table className="w-full">
                <thead className={`border-b ${isDark ? 'bg-slate-800/50 border-slate-700' : 'bg-slate-50 border-slate-200'}`}>
                  <tr>
                    <th className="px-6 py-4 text-left text-[10px] font-black uppercase tracking-widest opacity-40">Sigla</th>
                    <th className="px-6 py-4 text-left text-[10px] font-black uppercase tracking-widest opacity-40">Institución</th>
                    <th className="px-6 py-4 text-left text-[10px] font-black uppercase tracking-widest opacity-40">Categoría</th>
                    <th className="px-6 py-4 text-left text-[10px] font-black uppercase tracking-widest opacity-40">Ciudad</th>
                    <th className="px-6 py-4 text-left text-[10px] font-black uppercase tracking-widest opacity-40">Contratos</th>
                    ...
                    return (
                      ...
                      >
                        <td className="px-6 py-4">
                          <div className={`w-16 h-10 rounded-xl flex items-center justify-center font-black text-white text-[9px] shadow-lg relative tracking-tighter ${inst.active ? 'bg-slate-900' : 'bg-slate-400 opacity-50'}`}>
                            {inst.abbreviation}
                        ...
                        <td className="px-6 py-4">
                          <div>
                            <p className="text-sm font-black uppercase tracking-tight">{inst.name}</p>
                            <p className="text-[9px] opacity-40 font-bold flex items-center gap-1 mt-0.5">
                              <MapPin className="w-2.5 h-2.5" /> {inst.address}
                            </p>
                          </div>
                        </td>
                        <td className="px-6 py-4">
                          <span className="px-3 py-1.5 bg-blue-600/10 text-blue-600 rounded-lg text-[9px] font-black uppercase">{inst.category}</span>
                        </td>
                        <td className="px-6 py-4 text-sm font-bold opacity-60">{inst.city}</td>
                        <td className="px-6 py-4">
                          <div className="flex items-center gap-2">
                            <span className="text-lg font-black text-blue-600">{count}</span>
                            <span className="text-[8px] font-black uppercase opacity-30">Docs</span>
                          </div>
                        </td>
                        <td className="px-6 py-4">
                          <span className={`px-3 py-1.5 rounded-lg text-[9px] font-black uppercase ${inst.active ? 'bg-emerald-500/10 text-emerald-600' : 'bg-slate-500/10 text-slate-500'}`}>
                        ...
                        <td className="px-6 py-4">
                          <div className="flex items-center gap-2">
                            <button
                              onClick={() => { setSelectedInstId(inst.id); setGlobalViewMode('dashboard'); }}
            ...
            <div className={`px-6 py-4 border-t flex items-center justify-between ${isDark ? 'border-slate-800' : 'border-slate-200'}`}>
              <span className="text-sm opacity-40 font-bold">Mostrando {filteredInstitutions.length} de {institutions.length} instituciones</span>
            </div>
          </div>
        </div>
      )}
